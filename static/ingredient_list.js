$(document).ready(function () {
    ...
    $(".update").click(function(){
        var visible_parsed_ingredients = $('table > tbody  > tr:visible').map(row_data).toArray();
	...
	$(document).on("click", ".delete", function(){
        ...
        console.log(deleted_ingredients);
    ...
    function row_html(doc_id, ingredient, amount, unit) {
        return '<tr>' +
            ...
            '<td><input type="text" class="form-control" name="name" value="'+ingredient+'"></td>' +
            '<td>' + categories_select + '</td>' +
            '<td><input type="text" class="form-control" name="alii" value="'+unit+'"></td>' +
    ...
    function row_data(i, tr) {
        return {
          ...
          alii : $(tr).find('input[name="alii"]').val().split(",").map(s => s.trim()),
        }
    }
    ...
    ingredients.forEach(ingredient => {
        var row = $(row_html(ingredient.doc_id, ingredient.name, ingredient.category, ingredient.alii));
  ...
  const recipe_matches = function(recipe, query){
      ...
      ingredient_matches = recipe.ingredients.map(l => l.ingredient).some(str_matches);
